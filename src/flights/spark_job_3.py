...
def process(spark: SparkSession, flights_path: str, result_path: str):
    ...
    datamart = flights_fact \
        .where(flights_fact['ORIGIN_AIRPORT'].isNotNull()) \
        .groupBy(flights_fact['ORIGIN_AIRPORT']) \
        .agg(F.min(flights_fact['DEPARTURE_DELAY']).alias("min_delay"),
            F.max(flights_fact['DEPARTURE_DELAY']).alias("max_delay"),
            F.avg(flights_fact['DEPARTURE_DELAY']).alias("avg_delay"),
            F.corr(flights_fact['DEPARTURE_DELAY'], flights_fact['DAY_OF_WEEK']).alias("corr_delay2day_of_week")) \
