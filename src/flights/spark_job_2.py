...
def process(spark: SparkSession, flights_path: str, result_path: str):
    ...
    datamart = flights_fact \
        .where(flights_fact['ORIGIN_AIRPORT'].isNotNull() & flights_fact['DESTINATION_AIRPORT'].isNotNull()) \
        .groupBy(flights_fact['ORIGIN_AIRPORT'], flights_fact['DESTINATION_AIRPORT']) \
        .agg(F.count(flights_fact['TAIL_NUMBER']).alias("tail_count"), F.avg(flights_fact['AIR_TIME']).alias("avg_air_time")) \
